...
bool AABB::intersect(openvdb::math::Ray<float>&ray) {
  // intersection test for bounding box
  // ray distance for two intersection points are returned by pointers.
  float dir_frac_x = (ray.dir()[0] == 0.0) ? 1.0e32 : 1.0 / ray.dir()[0];
  float dir_frac_y = (ray.dir()[1] == 0.0) ? 1.0e32 : 1.0 / ray.dir()[1];
  float dir_frac_z = (ray.dir()[2] == 0.0) ? 1.0e32 : 1.0 / ray.dir()[2];

  float tx1 = (low_bnd[0] - ray.eye()[0]) * dir_frac_x;
  float tx2 = (upper_bnd[0] - ray.eye()[0]) * dir_frac_x;
  float ty1 = (low_bnd[1] - ray.eye()[1]) * dir_frac_y;
  float ty2 = (upper_bnd[1] - ray.eye()[1]) * dir_frac_y;
  float tz1 = (low_bnd[2] - ray.eye()[2]) * dir_frac_z;
  float tz2 = (upper_bnd[2] - ray.eye()[2]) * dir_frac_z;

  float t_in = std::max(std::max(std::min(tx1, tx2), std::min(ty1, ty2)),
                   std::min(tz1, tz2));
  float t_out = std::min(std::min(std::max(tx1, tx2), std::max(ty1, ty2)),
                    std::max(tz1, tz2));

  // When t_out < 0 and the ray is intersecting with AABB, the whole AABB is behind us
  t_in = std::max(t_in, ray.t0());
  t_out = std::min(t_out, ray.t1());
